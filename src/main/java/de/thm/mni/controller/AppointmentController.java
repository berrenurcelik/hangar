...
public class AppointmentController {
    ...
    /**
     * UC FB.7: saveAppointment Endpunkt (HandoverReturn)
     */
    @PostMapping("/save-handover")
    public ResponseEntity<HandoverReturnAppointment> saveHandoverAppointment(@RequestBody Map<String, Object> request) {
        try {
            ...
            HandoverReturnAppointment app = appointmentService.saveAppointment(hpId, acoId, date, status);
            return ResponseEntity.ok(app);
        } catch (Exception e) {
            return ResponseEntity.badRequest().build();
        }
